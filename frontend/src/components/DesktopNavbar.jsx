    ...
    <div className="hidden lg:flex items-center justify-between px-8 py-3 bg-white sticky top-0 z-50 shadow-sm border-b border-orange-50">
      ...
      <Link to="/" className="flex items-center gap-2.5 shrink-0">
        {appLogo ? (
          ...
          <div className="w-10 h-10 bg-[#fa6830] rounded-full flex items-center justify-center shadow-sm">
      ...
      <div className="flex items-center gap-1">
        {navLinks.map((link) => (
          <Link
            key={link.name}
            to={link.path}
            className="px-3 py-2 text-[13px] font-medium text-gray-600 hover:text-[#fa6830] hover:bg-orange-50 rounded-lg transition-colors whitespace-nowrap"
          >
            {link.name}
          </Link>
        ))}
      ...
      <div className="flex items-center gap-3 shrink-0">
        <button onClick={() => navigate('/user/notifications')} className="w-9 h-9 rounded-full bg-orange-50 flex items-center justify-center hover:bg-orange-100 transition-colors">
          <FiBell size={18} className="text-[#fa6830]" />
        </button>
        ...
        <button
          ...
          className="bg-[#fa6830] text-white font-bold text-[13px] px-6 py-2.5 rounded-full shadow-sm shadow-orange-200 hover:bg-[#e55923] active:scale-95 transition-all flex items-center gap-2"
        >
